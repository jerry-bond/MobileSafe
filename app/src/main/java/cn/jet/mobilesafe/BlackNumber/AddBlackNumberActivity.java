...
public class AddBlackNumberActivity extends Activity implements OnClickListener {
	...
	private BlackNumberDBOperator dao;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dao = new BlackNumberDBOperator(this);
		initView();
	}
		...
		case R.id.add_blacknum_btn:
			...
			if (TextUtils.isEmpty(number) || TextUtils.isEmpty(name)) {
				...
				if (mSmsCB.isChecked() & mTelCB.isChecked()) {
					// 两种拦截模式都选
					blackContactInfo.mode = Constants.BLACK_NUM_ALL;
				} else if (mSmsCB.isChecked() & !mTelCB.isChecked()) {
					// 短信拦截
					blackContactInfo.mode = Constants.BLACK_NUM_SMS;
				} else if (!mSmsCB.isChecked() & mTelCB.isChecked()) {
					// 电话拦截
					blackContactInfo.mode = Constants.BLACK_NUM_CALL;
				} else {
